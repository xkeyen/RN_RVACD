...
def miprincipal():
 ...
 df_dnis_1 = pd.read_csv(url_1,usecols=[14, 15],dtype=str)
 df_dnis_2 = pd.read_csv(url_2,skiprows=1,usecols=[2, 3],dtype=str)
 df_dnis_1 = df_dnis_1.dropna(how="all").reset_index(drop=True)
 df_dnis_2 = df_dnis_2.dropna(how="all").reset_index(drop=True)
 df_dnis_2 = df_dnis_2.rename(columns={"FN": "FECHA DE NACIMIENTO","DNI /CNV": "NUMERO DOCUMENTO"})
